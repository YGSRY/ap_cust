...
check_data() {
  ...
  if grep ' /data ' /proc/mounts | grep -vq 'tmpfs'; then
    ...
    $DATA && [ -d /cust/adb ] && touch /cust/adb/.rw && rm /cust/adb/.rw && DATA_DE=true
    $DATA_DE && [ -d /cust/adb/magisk ] || mkdir /cust/adb/magisk || DATA_DE=false
  fi
  set_nvbase "/data"
  $DATA || set_nvbase "/cache/data_adb"
  $DATA_DE && set_nvbase "/cust/adb"
}
...
remove_system_su() {
  ...
  elif [ -f /cache/su.img -o -f /data/su.img -o -d /data/su -o -d /cust/adb/su ]; then
    ui_print "- Removing systemless installed root"
    umount -l /su 2>/dev/null
    rm -rf /cache/su.img /data/su.img /data/su /cust/adb/su /cust/adb/suhide \
