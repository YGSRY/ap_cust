...
pub fn on_post_data_fs(superkey: Option<String>) -> Result<()> {
    ...
    let args = ["/cust/adb/ap/bin/magiskpolicy", "--magisk", "--live"];
    fork_for_result("/cust/adb/ap/bin/magiskpolicy", &args, &superkey);
    ...
    if safe_mode {
        // we should still mount modules.img to `/cust/adb/modules` in safe mode
...
fn run_uid_monitor() {
    ...
    let mut command = &mut Command::new("/cust/adb/apd");
    {
